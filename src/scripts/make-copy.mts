...
class FileTransferManager extends EventEmitter {
  ...
  private async executeTransferWithRetry(
    ...
    destPath: string,
    transferId: string,
    options: TransferOptions & { retryAttempts: number; retryDelay: number },
  ): Promise<void> {
    let attempts = 0;
    let stats: Stats;
    ...
    while (attempts < options.retryAttempts) {
      try {
        ...
        if (attempts === options.retryAttempts) {
          throw new Error(
            `Failed to transfer ${sourcePath} after ${attempts} attempts: ${error.message}`,
          );
        }
  ...
  async transferMultiple(
    files: string[],
    options: TransferOptions & { concurrency?: number } = {},
  ): Promise<void> {
